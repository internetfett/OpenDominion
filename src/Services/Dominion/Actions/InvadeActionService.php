...
class InvadeActionService
{
    ...
    $units_per_boat = 30;
    if($dominion->race->getPerkValue('boat_capacity') !== 0)
    {
        $units_per_boat += (int)$dominion->race->getPerkValue('boat_capacity');
    }
    ...
    protected function handleConversions(
        ...
        if (
            ...
            !in_array($dominion->race->name, ['Lycanthrope','Spirit', 'Undead','Sacred Order'], true) // todo: might want to check for conversion unit perks here, instead of hardcoded race names
        ) {
